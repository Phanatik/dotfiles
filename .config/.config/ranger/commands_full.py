...
class encode(Command):
    ...
    def execute(self):
        ...
        if target_file_ext in [".mkv",".mp4",".flv",".mov"]: 
            ...
            self.fm.notify("Let's encode the file " + target_filename + "!")
            ...
            shell_text = ["ffmpeg -i", target_file,"-vn -acodec flac",flac_file]
        ...
        self.fm.run(["echo",shell_text])


class extract(Command):
    # The so-called doc-string of the class will be visible in the built-in
    # help that is accessible by typing "?c" inside ranger.
    """:extract <filename> <directory to extract to>

    A command for extracting .zip and .7z files automatically with an optional directory parameter.
    """
    ...
    def execute(self):
        ...
        if self.arg(2):
            # self.rest(2) contains self.arg(2) and everything that follows
            extract_path = self.rest(2)

        target_file_ext = os.path.splitext(target_filename)[1]

        # Check if archive
        if target_file_ext in [".zip",".7z"]: 
            # This is a generic function to print text in ranger.
            self.fm.notify("Let's extract the file " + target_filename + "!")
            ...
            if target_file_ext == ".zip":
                shell_text = ["unzip", target_filename]
                if extract_path:
                    shell_text += ["-d", extract_path]
            
            if target_file_ext == ".7z":
                shell_text = ["7z","x",target_filename]
                if extract_path:
                    shell_text += ["-o",extract_path]
